...
*/
contract MushroomNFT is ERC721("Mushroom", "Mushroom"), Ownable, MushroomLib {
    mapping (uint256 => MushroomData) public mushroomData; // NFT Id -> Metadata
    mapping (uint256 => MushroomType) public mushroomTypes; // Species Id -> Metadata
    mapping (uint256 => bool) public mushroomTypeExists; // Species Id -> Exists

    /* ========== VIEWS ========== */

    // Mushrooms inherit their strength from their species
    function getMushroomData(uint256 tokenId) public view returns (MushroomData memory) {
        MushroomData memory data = mushroomData[tokenId];
        return data;
    }

    function getSpecies(uint256 speciesId) public view returns (MushroomType memory) {
        return mushroomTypes[speciesId];
    }

    function getRemainingMintableForSpecies(uint256 speciesId) public view returns (uint256) {
        MushroomType storage species = mushroomTypes[speciesId];
    ...
    function setMushroomLifespan(uint256 index, uint256 lifespan) public onlyOwner {
        MushroomData storage data = mushroomData[index];
    ...
    function _mintWithMetadata(address recipient, uint256 tokenId, uint256 speciesId, uint256 lifespan) internal {
        ...
        mushroomData[tokenId] = MushroomData(speciesId, species.strength, lifespan);
    ...
    function _clearMushroomData(uint256 tokenId) internal {
        MushroomData storage data = mushroomData[tokenId];
        MushroomType storage species = mushroomTypes[data.species];     

        species.minted = species.minted.sub(1);
    }
    function setMushroomType(uint256 speciesId, MushroomType memory mType) public onlyOwner {
